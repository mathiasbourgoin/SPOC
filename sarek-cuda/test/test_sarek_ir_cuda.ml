...
let test_gen_stmt_block () =
  ...
  let code = gen_stmt_str s in
  Alcotest.(check bool) "Block has opening brace" true (String.contains code '{') ;
  Alcotest.(check bool) "Block has closing brace" true (String.contains code '}')
