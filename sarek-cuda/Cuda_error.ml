...
let library_not_found library paths =
  Plugin (Library_not_found {library; paths})
...
let to_string = function
  | Codegen err -> (
      ...
      | Invalid_memory_space {decl; space} ->
          Printf.sprintf
            "Invalid memory space '%s' for declaration: %s"
            space
            decl)
  | Runtime err -> (
      match err with
...
let with_default ~default f = try f () with Cuda_error _ -> default

(** Convert error to Result type *)
let to_result f = try Ok (f ()) with Cuda_error err -> Error err
