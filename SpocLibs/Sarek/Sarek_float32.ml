...
(* Pure OCaml fallback if C stubs not available *)
let float32_of_float_ocaml x =
...
let to_float32 = float32_of_float_ocaml
...
let check_overflow name result =
  ...
  | Exception ->
      if result = infinity then
        raise (Float32_overflow name)
      else if result = neg_infinity then
        raise (Float32_overflow (name ^ " (negative)"))
      else result

(** Check for underflow *)
let check_underflow name x result =
...
let exp x =
  if x > max_exp_input then
    check_overflow "exp" infinity
  else
    to_float32 (Stdlib.exp x) |> check_overflow "exp"

let log x =
  if x <= 0. then
    check_overflow "log" neg_infinity
  else
    to_float32 (Stdlib.log x)

let log10 x =
  if x <= 0. then
    check_overflow "log10" neg_infinity
  else
    to_float32 (Stdlib.log10 x)
