...
let devices = measure_time (Devices.init ) "init"
...
let _ =
  ...
  and arg4 = ("-d", Arg.Int (fun i -> devId := i), "Choose the device (default: all)")
  in
  Arg.parse [arg0; arg1; arg2; arg3; arg4] (fun s -> ()) "";

  if !filename = "" then
    Arg.usage [arg0; arg1; arg2; arg3; arg4]  "Nearest Neighbor Usage\n NN.asm -f [filname] -r [int] -lat [int] -lng [int] -d [int]";
  ...
  let distances = Vector.create Vector.float32 numRecords
