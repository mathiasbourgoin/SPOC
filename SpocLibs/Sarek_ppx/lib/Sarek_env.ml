...
type type_info =
  ...
  | TIVariant of {
      ti_name: string;
      ti_constrs: (string * typ option) list;  (** constructor name, optional arg type *)
    }
...
type t = {
  ...
  constructors: (string * typ option) StringMap.t;  (** constr -> (type_name, arg_type) *)
  fields: (string * int * typ * bool) StringMap.t;   (** field -> (type_name, index, type, mutable) *)
...
let add_type name info env =
  ...
  | TIRecord { ti_fields; ti_name; _ } ->
    ...
    let _, fields = List.fold_left (fun (idx, fields) (fname, ftyp, fmut) ->
        (idx + 1, StringMap.add fname (ti_name, idx, ftyp, fmut) fields)
      ) (0, env.fields) ti_fields in
    { env with fields }
  | TIVariant { ti_constrs; ti_name; _ } ->
    let env = { env with types = StringMap.add name info env.types } in
    (* Also add constructor mappings *)
    let constrs = List.fold_left (fun constrs (cname, carg) ->
        StringMap.add cname (ti_name, carg) constrs
      ) env.constructors ti_constrs in
...
let with_stdlib env =
  ...
  let env = add_intrinsic_const "thread_idx_x"
      { const_type = t_int32; const_cuda = "threadIdx.x"; const_opencl = "get_local_id(0)" } env in
  let env = add_intrinsic_const "thread_idx_y"
      { const_type = t_int32; const_cuda = "threadIdx.y"; const_opencl = "get_local_id(1)" } env in
  let env = add_intrinsic_const "thread_idx_z"
      { const_type = t_int32; const_cuda = "threadIdx.z"; const_opencl = "get_local_id(2)" } env in

  let env = add_intrinsic_const "block_idx_x"
      { const_type = t_int32; const_cuda = "blockIdx.x"; const_opencl = "get_group_id(0)" } env in
  let env = add_intrinsic_const "block_idx_y"
      { const_type = t_int32; const_cuda = "blockIdx.y"; const_opencl = "get_group_id(1)" } env in
  let env = add_intrinsic_const "block_idx_z"
      { const_type = t_int32; const_cuda = "blockIdx.z"; const_opencl = "get_group_id(2)" } env in

  let env = add_intrinsic_const "block_dim_x"
      { const_type = t_int32; const_cuda = "blockDim.x"; const_opencl = "get_local_size(0)" } env in
  let env = add_intrinsic_const "block_dim_y"
      { const_type = t_int32; const_cuda = "blockDim.y"; const_opencl = "get_local_size(1)" } env in
  let env = add_intrinsic_const "block_dim_z"
      { const_type = t_int32; const_cuda = "blockDim.z"; const_opencl = "get_local_size(2)" } env in

  let env = add_intrinsic_const "grid_dim_x"
      { const_type = t_int32; const_cuda = "gridDim.x"; const_opencl = "get_num_groups(0)" } env in
  let env = add_intrinsic_const "grid_dim_y"
      { const_type = t_int32; const_cuda = "gridDim.y"; const_opencl = "get_num_groups(1)" } env in
  let env = add_intrinsic_const "grid_dim_z"
      { const_type = t_int32; const_cuda = "gridDim.z"; const_opencl = "get_num_groups(2)" } env in

  let env = add_intrinsic_const "global_thread_id"
      { const_type = t_int32;
        const_cuda = "(blockIdx.x*blockDim.x+threadIdx.x)";
        const_opencl = "get_global_id(0)" } env in

  (* Std module functions *)
  let env = add_intrinsic_fun "block_barrier"
      { intr_type = t_fun [t_unit] t_unit;
        intr_cuda = "__syncthreads()";
        intr_opencl = "barrier(CLK_LOCAL_MEM_FENCE)" } env in

  let env = add_intrinsic_fun "return"
      { intr_type = t_fun [t_unit] t_unit;
        intr_cuda = "return";
        intr_opencl = "return" } env in

  (* Type conversion functions *)
  let env = add_intrinsic_fun "float"
      { intr_type = t_fun [t_int32] t_float32;
        intr_cuda = "(float)";
        intr_opencl = "(float)" } env in

  let env = add_intrinsic_fun "float64"
      { intr_type = t_fun [t_int32] t_float64;
        intr_cuda = "(double)";
        intr_opencl = "(double)" } env in

  let env = add_intrinsic_fun "int_of_float"
      { intr_type = t_fun [t_float32] t_int32;
        intr_cuda = "(int)";
        intr_opencl = "(int)" } env in

  let env = add_intrinsic_fun "int_of_float64"
      { intr_type = t_fun [t_float64] t_int32;
        intr_cuda = "(int)";
        intr_opencl = "(int)" } env in

  (* Math.Float32 functions *)
  let float32_funs = [
    ("sin", "sinf", "sin");
    ("cos", "cosf", "cos");
    ("tan", "tanf", "tan");
  ...
  let env = List.fold_left (fun env (name, cuda, opencl) ->
      add_intrinsic_fun name
        { intr_type = t_fun [t_float32] t_float32;
          intr_cuda = cuda;
          intr_opencl = opencl } env
    ) env float32_funs in
  ...
  let env = List.fold_left (fun env (name, cuda, opencl) ->
      add_intrinsic_fun name
        { intr_type = t_fun [t_float32; t_float32] t_float32;
          intr_cuda = cuda;
          intr_opencl = opencl } env
    ) env float32_bin_funs in

  (* Math.Float64 functions (same as Float32 but without f suffix) *)
  let float64_funs = [
    ("sin64", "sin", "sin");
    ("cos64", "cos", "cos");
    ("tan64", "tan", "tan");
  ...
  let env = List.fold_left (fun env (name, cuda, opencl) ->
      add_intrinsic_fun name
        { intr_type = t_fun [t_float64; t_float64] t_float64;
          intr_cuda = cuda;
          intr_opencl = opencl } env
    ) env float64_bin_funs in

  (* Integer math functions *)
  let env = add_intrinsic_fun "logical_and"
      { intr_type = t_fun [t_int32; t_int32] t_int32;
        intr_cuda = "logical_and";
        intr_opencl = "logical_and" } env in

  let env = add_intrinsic_fun "xor"
      { intr_type = t_fun [t_int32; t_int32] t_int32;
        intr_cuda = "spoc_xor";
        intr_opencl = "spoc_xor" } env in

  let env = add_intrinsic_fun "spoc_powint"
      { intr_type = t_fun [t_int32; t_int32] t_int32;
        intr_cuda = "spoc_powint";
        intr_opencl = "spoc_powint" } env in
...
let pp_env fmt env =
  ...
  StringMap.iter (fun name info ->
      Format.fprintf fmt "  %s : %a (param=%b, idx=%d)@."
        name pp_typ info.vi_type info.vi_is_param info.vi_index
    ) env.vars;
  Format.fprintf fmt "Intrinsic constants:@.";
  StringMap.iter (fun name info ->
      Format.fprintf fmt "  %s : %a (cuda=%s, opencl=%s)@."
        name pp_typ info.const_type info.const_cuda info.const_opencl
    ) env.intrinsic_consts;
  Format.fprintf fmt "Intrinsic functions:@.";
  StringMap.iter (fun name info ->
      Format.fprintf fmt "  %s : %a (cuda=%s, opencl=%s)@."
        name pp_typ info.intr_type info.intr_cuda info.intr_opencl
    ) env.intrinsic_funs
