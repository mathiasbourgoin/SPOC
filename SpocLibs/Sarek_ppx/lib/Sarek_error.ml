...
let pp_error fmt = function
  ...
  | Unbound_type (name, _) ->
    Format.fprintf fmt "Unbound type: %s" name
  | Type_mismatch { expected; got; _ } ->
    Format.fprintf fmt "Type mismatch: expected %a, got %a"
      pp_typ expected pp_typ got
  ...
  | Wrong_arity { expected; got; _ } ->
    Format.fprintf fmt "Wrong number of arguments: expected %d, got %d"
      expected got
  | Not_a_vector (t, _) ->
  ...
  | Invalid_kernel (msg, _) ->
    Format.fprintf fmt "Invalid kernel: %s" msg
  | Duplicate_field (name, _) ->
    Format.fprintf fmt "Duplicate field: %s" name
...
let pp_error_with_loc fmt e =
  ...
  Format.fprintf fmt "%s:%d:%d: %a"
    loc.loc_file loc.loc_line loc.loc_col pp_error e
...
let combine_results results =
  let rec aux acc = function
    ...
    | Error es :: rest ->
      ...
      let more_errors = List.filter_map (function
          | Ok _ -> None
          | Error es -> Some es
        ) rest in
