...
let evar_qualified ~loc path name =
  let lid = List.fold_left (fun acc m -> Ldot (acc, m)) (Lident (List.hd path)) (List.tl path @ [name]) in
...
let quote_list ~loc quote_elem elems =
  List.fold_right (fun elem acc ->
      [%expr [%e quote_elem ~loc elem] :: [%e acc]]
    ) elems [%expr []]
...
let rec quote_case ~loc ((i, opt, body) : Kirc_Ast.case) : expression =
  let opt_expr = match opt with
    | None -> [%expr None]
    | Some (s1, s2, id, s3) ->
      [%expr Some ([%e quote_string ~loc s1], [%e quote_string ~loc s2],
                   [%e quote_int ~loc id], [%e quote_string ~loc s3])]
  in
