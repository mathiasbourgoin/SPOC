...
type typ =
  ...
  | TVariant of string * (string * typ option) list  (** Variant type: name, constructors *)
...
let rec occurs (id : int) (t : typ) : bool =
  ...
  | TVar { contents = Link _ } -> assert false (* repr should have followed links *)
  | TPrim _ -> false
  | TVec t -> occurs id t
  ...
  | TVariant (_, constrs) ->
    List.exists (function (_, None) -> false | (_, Some t) -> occurs id t) constrs
...
let rec unify (t1 : typ) (t2 : typ) : (unit, unify_error) result =
  ...
  if t1 == t2 then Ok ()
    ...
    | TRecord (n1, fields1), TRecord (n2, fields2) when n1 = n2 ->
...
let rec pp_typ fmt t =
  ...
  | TFun (args, ret) ->
    Format.fprintf fmt "(%a) -> %a"
      (Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt " * ") pp_typ) args
      pp_typ ret
  | TRecord (name, fields) ->
...
let rec type_of_type_expr (te : Sarek_ast.type_expr) : typ =
  ...
  | Sarek_ast.TEConstr (name, [elem]) when String.ends_with ~suffix:"vector" name ->
  ...
  | Sarek_ast.TEConstr (name, args) ->
    ...
    TRecord (name, [])  (* Placeholder - will be resolved by typer *)
  | Sarek_ast.TEArrow (a, b) ->
    TFun ([type_of_type_expr a], type_of_type_expr b)
  | Sarek_ast.TETuple ts ->
    TTuple (List.map type_of_type_expr ts)
