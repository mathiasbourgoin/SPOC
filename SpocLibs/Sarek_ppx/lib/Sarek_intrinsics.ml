...
type intrinsic = {
  ...
  i_cuda : string;
  i_opencl : string;
  i_module : module_path;
      (** Which module this belongs to, e.g., ["Std"] or ["Math"; "Float32"] *)
}
...
let std_consts : intrinsic list =
  [
    ...
    };
    {
      ...
      i_type = t_int32;
      i_cuda = "gridDim.z";
      i_opencl = "get_num_groups(2)";
      i_module = ["Std"];
    };
    (* Convenience *)
    {
      i_name = "global_thread_id";
      i_type = t_int32;
      i_cuda = "(blockIdx.x*blockDim.x+threadIdx.x)";
      i_opencl = "get_global_id(0)";
      i_module = ["Std"];
    };
  ]
...
let std_funs : intrinsic list =
  [
    (* Synchronization *)
    {
      i_name = "block_barrier";
      i_type = t_fun [t_unit] t_unit;
      i_cuda = "__syncthreads()";
      i_opencl = "barrier(CLK_LOCAL_MEM_FENCE)";
      i_module = ["Std"];
    };
    (* Type conversions *)
    {
      i_name = "float";
      i_type = t_fun [t_int32] t_float32;
      i_cuda = "(float)";
      i_opencl = "(float)";
      i_module = ["Std"];
    };
    {
      i_name = "float64";
      i_type = t_fun [t_int32] t_float64;
      i_cuda = "(double)";
      i_opencl = "(double)";
      i_module = ["Std"];
    };
    {
      i_name = "int_of_float";
      i_type = t_fun [t_float32] t_int32;
      i_cuda = "(int)";
      i_opencl = "(int)";
      i_module = ["Std"];
    };
    {
      i_name = "int_of_float64";
      i_type = t_fun [t_float64] t_int32;
      i_cuda = "(int)";
      i_opencl = "(int)";
      i_module = ["Std"];
    };
...
let all_intrinsics =
  std_consts @ std_funs @ math_float32_funs @ math_float64_funs @ math_int_funs
  @ vector_funs
...
let is_intrinsic_fun (name : string) : bool =
  let all_funs =
    std_funs @ math_float32_funs @ math_float64_funs @ math_int_funs
  in
...
let module_aliases : (string list * module_path) list =
  [
    ...
    (["Float32"], ["Math"; "Float32"]);
    (* Alias *)
    (["Float64"], ["Math"; "Float64"]);
    (* Alias *)
