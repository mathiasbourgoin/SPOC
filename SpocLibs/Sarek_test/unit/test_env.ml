...
let test_add_find_var () =
  ...
  Alcotest.(check bool) "find_var returns Some for added var" true
    (match find_var "x" env with Some _ -> true | None -> false);

  Alcotest.(check bool) "find_var returns None for unknown var" true
    (match find_var "y" env with Some _ -> false | None -> true);

  match find_var "x" env with
  | Some found_info ->
    Alcotest.(check bool) "found var has correct type" true
      (match found_info.vi_type with TPrim TInt32 -> true | _ -> false);
    Alcotest.(check bool) "found var has correct is_param" true found_info.vi_is_param
  | None ->
    Alcotest.fail "var x should be found"
...
let test_var_shadowing () =
  ...
  match find_var "x" env with
  | Some found ->
    Alcotest.(check bool) "shadowed var has new type" true
      (match found.vi_type with TPrim TFloat32 -> true | _ -> false);
    Alcotest.(check bool) "shadowed var has new mutability" true found.vi_mutable
  | None ->
    Alcotest.fail "var x should be found"
...
let test_intrinsic_consts () =
  ...
  Alcotest.(check bool) "thread_idx_x is found" true
    (match find_intrinsic_const "thread_idx_x" env with Some _ -> true | None -> false);

  Alcotest.(check bool) "block_idx_x is found" true
    (match find_intrinsic_const "block_idx_x" env with Some _ -> true | None -> false);

  Alcotest.(check bool) "global_thread_id is found" true
    (match find_intrinsic_const "global_thread_id" env with Some _ -> true | None -> false);

  match find_intrinsic_const "thread_idx_x" env with
  | Some info ->
    Alcotest.(check bool) "thread_idx_x has int32 type" true
      (match info.const_type with TPrim TInt32 -> true | _ -> false);
    Alcotest.(check string) "thread_idx_x has correct cuda code" "threadIdx.x" info.const_cuda;
    Alcotest.(check string) "thread_idx_x has correct opencl code" "get_local_id(0)" info.const_opencl
  | None ->
    Alcotest.fail "thread_idx_x should be found"

(* Test intrinsic functions *)
let test_intrinsic_funs () =
  let env = with_stdlib empty in

  Alcotest.(check bool) "block_barrier is found" true
    (match find_intrinsic_fun "block_barrier" env with Some _ -> true | None -> false);

  Alcotest.(check bool) "sin is found" true
    (match find_intrinsic_fun "sin" env with Some _ -> true | None -> false);

  Alcotest.(check bool) "cos is found" true
  ...
  | Some info ->
    (match info.intr_type with
     ...
     | _ ->
       Alcotest.fail "sin should have type float32 -> float32");
    Alcotest.(check string) "sin has correct cuda code" "sinf" info.intr_cuda
  | None ->
    Alcotest.fail "sin should be found"
...
let test_lookup () =
  ...
  (match lookup "my_var" env with
   | LVar info ->
     Alcotest.(check bool) "lookup finds var with correct type" true
       (match info.vi_type with TPrim TInt32 -> true | _ -> false)
   | _ ->
     Alcotest.fail "my_var should be found as LVar");

  (* Test lookup of intrinsic const *)
  (match lookup "thread_idx_x" env with
   | LIntrinsicConst _ -> Alcotest.(check pass) "lookup finds intrinsic const" () ()
...
let () =
  Alcotest.run "Sarek_env" [
    ...
    "lookup", [
      Alcotest.test_case "lookup function" `Quick test_lookup;
    ];
    "custom_types", [
    ...
    "levels", [
      Alcotest.test_case "level management" `Quick test_levels;
    ];
  ]
