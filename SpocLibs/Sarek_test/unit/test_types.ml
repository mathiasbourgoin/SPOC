...
let test_prim_types () =
  Alcotest.(check bool) "t_unit is TPrim TUnit" true
    (match t_unit with TPrim TUnit -> true | _ -> false);
  Alcotest.(check bool) "t_bool is TPrim TBool" true
    (match t_bool with TPrim TBool -> true | _ -> false);
  Alcotest.(check bool) "t_int32 is TPrim TInt32" true
    (match t_int32 with TPrim TInt32 -> true | _ -> false);
  Alcotest.(check bool) "t_float32 is TPrim TFloat32" true
...
let test_type_constructors () =
  ...
  Alcotest.(check bool) "t_vec creates TVec" true
    (match vec_int with TVec (TPrim TInt32) -> true | _ -> false);

  let arr_float = t_arr t_float32 Local in
  Alcotest.(check bool) "t_arr creates TArr" true
    (match arr_float with TArr (TPrim TFloat32, Local) -> true | _ -> false);

  let fn = t_fun [t_int32; t_float32] t_bool in
  Alcotest.(check bool) "t_fun creates TFun" true
    (match fn with TFun ([TPrim TInt32; TPrim TFloat32], TPrim TBool) -> true | _ -> false)
...
let test_fresh_tvar () =
  ...
  Alcotest.(check bool) "fresh tvars are different" true
    (match t1, t2 with
     | TVar { contents = Unbound (id1, _) }, TVar { contents = Unbound (id2, _) } ->
       id1 <> id2
     | _ -> false)
