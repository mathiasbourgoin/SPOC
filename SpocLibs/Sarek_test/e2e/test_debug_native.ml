...
let simple_write_kernel =
  [%kernel
    fun (output : float32 vector) (n : int32) ->
      ...
      if tid < n then
        output.(tid) <- Float32.of_float 42.0]
...
let tid_kernel =
  [%kernel
    fun (output : float32 vector) (n : int32) ->
      ...
      let tid = global_thread_id in
      if tid < n then
        output.(tid) <- Float32.of_int (Int32.to_int tid)]

let () =
  let devs = V2_Device.init ~frameworks:["Native"] () in
  let native_dev = devs.(0) in
  
  Printf.printf "Testing on %s\n%!" native_dev.V2_Device.name;
  
  let n = 16 in
  
  (* Test 1: simple write kernel *)
  Printf.printf "\n--- Test 1: Simple write (should see 42.0) ---\n%!";
  let output1 = V2_Vector.create V2_Vector.float32 n in
  for i = 0 to n-1 do V2_Vector.set output1 i 0.0 done;
  
  let _, kirc1 = simple_write_kernel in
  let ir1 = match kirc1.Sarek.Kirc_types.body_v2 with Some ir -> ir | None -> failwith "no ir" in
  ...
  Execute.run_vectors
    ...
    ();
  V2_Transfer.flush native_dev;
  
  Printf.printf "Result: [";
  for i = 0 to min 10 (n-1) do
    Printf.printf " %.1f" (V2_Vector.get output1 i)
  done;
  Printf.printf " ]\n%!";
  
  (* Test 2: tid kernel *)
  Printf.printf "\n--- Test 2: Thread ID (should see 0, 1, 2, ...) ---\n%!";
  let output2 = V2_Vector.create V2_Vector.float32 n in
  for i = 0 to n-1 do V2_Vector.set output2 i 0.0 done;
  
  let _, kirc2 = tid_kernel in
  let ir2 = match kirc2.Sarek.Kirc_types.body_v2 with Some ir -> ir | None -> failwith "no ir" in
  ...
  Printf.printf "Result: [";
  for i = 0 to min 10 (n-1) do
    Printf.printf " %.1f" (V2_Vector.get output2 i)
  done;
  Printf.printf " ]\n%!";
