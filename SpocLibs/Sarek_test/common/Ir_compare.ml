...
let rec ir_equal (ir1 : k_ext) (ir2 : k_ext) : bool =
  ...
  | Custom (t1, i1, s1), Custom (t2, i2, s2) ->
    t1 = t2 && i1 = i2 && s1 = s2
  | CustomVar (t1, c1, s1), CustomVar (t2, c2, s2) ->
    t1 = t2 && c1 = c2 && s1 = s2
  | RecGet (r1, f1), RecGet (r2, f2) ->
    ir_equal r1 r2 && f1 = f2
  | Record (n1, fs1), Record (n2, fs2) ->
    n1 = n2 && List.length fs1 = List.length fs2 &&
    List.for_all2 ir_equal fs1 fs2
  | Constr (t1, c1, as1), Constr (t2, c2, as2) ->
    t1 = t2 && c1 = c2 && List.length as1 = List.length as2 &&
    List.for_all2 ir_equal as1 as2
  | Match (t1, s1, cs1), Match (t2, s2, cs2) ->
    t1 = t2 && ir_equal s1 s2 && Array.length cs1 = Array.length cs2 &&
    Array.for_all2 case_equal cs1 cs2
  | App (f1, as1), App (f2, as2) ->
    ir_equal f1 f2 && Array.length as1 = Array.length as2 &&
  ...
  | Pragma (os1, b1), Pragma (os2, b2) ->
    os1 = os2 && ir_equal b1 b2
  | CastDoubleVar (i1, s1), CastDoubleVar (i2, s2) ->
    i1 = i2 && s1 = s2
...
and case_equal (i1, o1, b1) (i2, o2, b2) =
  i1 = i2 && o1 = o2 && ir_equal b1 b2
...
let diff_summary (expected : k_ext) (got : k_ext) : string =
  ...
  if diffs = [] then "IR trees are identical"
    ...
    Format.asprintf "Found %d differences:@.%a"
      (List.length diffs) pp_diffs diffs
