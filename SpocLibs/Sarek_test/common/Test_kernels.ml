...
type test_kernel = {
  ...
  tags: string list;  (** Categories: "basic", "control_flow", "custom_types", etc. *)
}
...
let kernels_by_tag tag =
  List.filter (fun k -> List.mem tag k.tags) all_kernels

(** Get kernel by ID *)
let find_kernel id =
  List.find_opt (fun k -> k.id = id) all_kernels
