...
let find_reference_file () =
  ...
  with Not_found ->
    failwith "Could not find reference_ir.txt"
...
let () =
  ...
  let missing = List.filter (fun k -> not (List.mem k found_kernels)) expected_kernels in
  let extra = List.filter (fun k -> not (List.mem k expected_kernels)) found_kernels in
  ...
  if success then begin
    ...
    Printf.printf "       %d kernels available for comparison testing\n" (List.length found_kernels);
    exit 0
  end else begin
