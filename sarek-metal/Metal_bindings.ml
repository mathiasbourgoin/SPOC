...
let objc_msgSend_lazy =
  lazy (foreign ~from:(get_objc_lib ()) "objc_msgSend" (ptr void @-> ptr void @-> returning (ptr void)))
...
let sel_registerName_lazy =
  lazy (foreign ~from:(get_objc_lib ()) "sel_registerName" (string @-> returning (ptr void)))

let sel_registerName name = Lazy.force sel_registerName_lazy name

(** objc_getClass - get a class by name *)
let objc_getClass_lazy =
  lazy (foreign ~from:(get_objc_lib ()) "objc_getClass" (string @-> returning (ptr void)))
...
let foundation_lib : Dl.library option Lazy.t =
  lazy
    (try
       Some
         (Dl.dlopen
            ~filename:"/System/Library/Frameworks/Foundation.framework/Foundation"
            ~flags:[Dl.RTLD_LAZY])
     with _ -> None)
...
let nsstring_from_cstring str =
  ...
  let init_fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> string @-> returning (ptr void)) in
...
let cstring_from_nsstring nsstr =
  ...
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> returning string) in
  fn nsstr sel
...
let nserror_description err =
  if is_null err then "No error"
    ...
    if is_null desc then "Unknown error"
    else cstring_from_nsstring desc

(** {1 Metal Device API} *)

(** MTLCreateSystemDefaultDevice - get default GPU *)
let mtl_create_system_default_device_lazy =
  lazy (foreign ~from:(get_metal_lib ()) "MTLCreateSystemDefaultDevice"
...
let mtl_copy_all_devices_lazy =
  lazy (foreign ~from:(get_metal_lib ()) "MTLCopyAllDevices"
          (void @-> returning (ptr void))) (* Returns NSArray *)

let mtl_copy_all_devices () =
  Lazy.force mtl_copy_all_devices_lazy ()
...
let mtl_device_name dev =
  ...
  if is_null nsstr then "Unknown Device"
  else cstring_from_nsstring nsstr

let mtl_device_max_threads_per_threadgroup dev =
  let sel = sel_registerName "maxThreadsPerThreadgroup" in
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend_stret"
      (ptr void @-> ptr void @-> ptr void @-> returning void) in
...
let mtl_device_new_buffer_with_length dev length options =
  ...
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> uint64_t @-> uint64_t @-> returning mtl_buffer) in
...
let mtl_device_new_library_with_source dev source _options =
  ...
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> ns_string @-> ptr void @-> ptr (ptr void) @-> returning mtl_library) in
...
let mtl_library_new_function_with_name lib fname =
  ...
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> ns_string @-> returning mtl_function) in
  fn lib sel fname_ns
...
let mtl_device_new_compute_pipeline_state dev func =
  ...
  let error_ptr = allocate (ptr void) null in
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> mtl_function @-> ptr (ptr void) @-> returning mtl_compute_pipeline_state) in
...
let mtl_compute_command_encoder_set_compute_pipeline_state encoder pso =
  ...
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> mtl_compute_pipeline_state @-> returning void) in
  fn encoder sel pso
...
let mtl_compute_command_encoder_set_buffer encoder buffer offset index =
  ...
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> mtl_buffer @-> uint64_t @-> uint64_t @-> returning void) in
  fn encoder sel buffer (Unsigned.UInt64.of_int offset) (Unsigned.UInt64.of_int index)

let mtl_compute_command_encoder_set_bytes encoder bytes length index =
  let sel = sel_registerName "setBytes:length:atIndex:" in
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> ptr void @-> uint64_t @-> uint64_t @-> returning void) in
  fn encoder sel bytes (Unsigned.UInt64.of_int length) (Unsigned.UInt64.of_int index)

let mtl_compute_command_encoder_dispatch_threads encoder threads threads_per_threadgroup =
  let sel = sel_registerName "dispatchThreads:threadsPerThreadgroup:" in
  let fn = foreign ~from:(get_objc_lib ()) "objc_msgSend"
      (ptr void @-> ptr void @-> mtl_size @-> mtl_size @-> returning void) in
