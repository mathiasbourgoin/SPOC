...
(* camlp4 compatibility layer *)
type ctyp = core_type
type ident = longident
...
type  customtypes =
  ...
  | KVariant of (string * ctyp option)  list
...
let rec ktyp_to_string = function
  ...
  | TArr (k,_m) -> (ktyp_to_string k)^" array"
...
type k_expr =
  | Open of location * ident * kexpr
  | App of location * kexpr * kexpr list
  | Acc of location * kexpr * kexpr
  | VecSet of location*kexpr*kexpr
  | VecGet of location*kexpr*kexpr
  | ArrSet of location*kexpr*kexpr
  | ArrGet of location*kexpr*kexpr
  | Seq of location*kexpr*kexpr
  | Fun of location*expr*ktyp*cfun*(string list)
  | Bind of location*kexpr*kexpr*kexpr*bool
  | Plus32 of location*kexpr*kexpr
  | Plus64 of location*kexpr*kexpr
  | PlusF32 of location*kexpr*kexpr
  | PlusF64 of location*kexpr*kexpr
  | Record of location*field list
  | RecGet of location*kexpr*ident
  | RecSet of location*kexpr*kexpr

  | Map of location * kexpr*kexpr*kexpr


  | Min32 of location*kexpr*kexpr
  | Min64 of location*kexpr*kexpr
  | MinF32 of location*kexpr*kexpr
  | MinF64 of location*kexpr*kexpr

  | Mul32 of location*kexpr*kexpr
  | Mul64 of location*kexpr*kexpr
  | MulF32 of location*kexpr*kexpr
  | MulF64 of location*kexpr*kexpr

  | Mod of location*kexpr*kexpr

  | Div32 of location*kexpr*kexpr
  | Div64 of location*kexpr*kexpr
  | DivF32 of location*kexpr*kexpr
  | DivF64 of location*kexpr*kexpr

  | Id of location*ident
  | Int of location*string
  | Int32 of location*string
  | Int64 of location*string
  | Float of location*string
  | Float32 of location*string
  | Float64 of location*string
  | BoolNot of location*kexpr
  | BoolAnd of location*kexpr*kexpr
  | BoolOr of location*kexpr*kexpr
  | BoolEq of location*kexpr*kexpr
  | BoolEq32 of location*kexpr*kexpr
  | BoolEq64 of location*kexpr*kexpr
  | BoolEqF32 of location*kexpr*kexpr
  | BoolEqF64 of location*kexpr*kexpr
  | BoolLt of location*kexpr*kexpr
  | BoolLt32 of location*kexpr*kexpr
  | BoolLt64 of location*kexpr*kexpr
  | BoolLtF32 of location*kexpr*kexpr
  | BoolLtF64 of location*kexpr*kexpr

  | BoolLtE of location*kexpr*kexpr
  | BoolLtE32 of location*kexpr*kexpr
  | BoolLtE64 of location*kexpr*kexpr
  | BoolLtEF32 of location*kexpr*kexpr
  | BoolLtEF64 of location*kexpr*kexpr

  | BoolGt of location*kexpr*kexpr
  | BoolGt32 of location*kexpr*kexpr
  | BoolGt64 of location*kexpr*kexpr
  | BoolGtF32 of location*kexpr*kexpr
  | BoolGtF64 of location*kexpr*kexpr

  | BoolGtE of location*kexpr*kexpr
  | BoolGtE32 of location*kexpr*kexpr
  | BoolGtE64 of location*kexpr*kexpr
  | BoolGtEF32 of location*kexpr*kexpr
  | BoolGtEF64 of location*kexpr*kexpr

  | Ife of location * kexpr * kexpr * kexpr
  | If of location * kexpr * kexpr
  | Match of location * kexpr * (case list)
  | DoLoop of location * kexpr * kexpr * kexpr * kexpr
  | While of location * kexpr * kexpr
  | End of location*kexpr
  | Ref of location*kexpr

  | ModuleAccess of location * string * kexpr
  | True of location | False of location

  | TypeConstraint of location * kexpr * ktyp
  | Nat of location * expr
  | Pragma of location * string list * kexpr
  | Noop

and case =  location * pattern * kexpr (* | patt -> e *)
and field = location*ident*kexpr
...
let rec is_unknown t =
  let rec app_return_type = function
    | TApp (_,(TApp (_a,b))) -> app_return_type b
  ...
  | TArr (TUnknown, _)
  | TVec TAny -> true
  | TApp (_a,b) -> app_return_type b
  | _ -> false


let update_type (value : kexpr) t =
  ...
  | _t1, _t2 ->
    ...
    if not (is_unknown t) then
      if not (is_unknown value.t)  && value.t <> t then
        (assert (not debug); raise (TypeError {loc = value.loc; txt = t, value.t}) )
      else
        (
          if value.t <> t then
            (value.t <- t;
...
let string_of_ident i =
  Ppxlib.Longident.name i



let k_expr_to_string = function
  | App (_,{t=_;e=Id(_l,s);loc=_},_) -> ("App "^(string_of_ident s))
  | App _ -> "App"
  | Acc _ -> "Acc"
  | VecSet _ -> "VecSet"
  | VecGet _ -> "VecGet"
...
let expr_of_patt p =
  match p with
  | Ppat_construct  ({txt= Lident txt; _}, _) -> Ast_builder.Default.evar txt
...
let (arg_list : expression list ref ) = ref []

let n_lifted_vals = ref 0

let string_of_loc {pos_fname; pos_lnum; pos_bol; pos_cnum} =
  Format.sprintf "%s: %d - %d" pos_fname pos_lnum (pos_cnum - pos_bol)

let rec ktyp_of_typ = function
  | [%type: unit] -> TUnit
  | [%type: int ] | [%type:  int32 ] -> TInt32
  | [%type: int64 ]  -> TInt64
  | [%type: float ] | [%type: float32 ] -> TFloat32
  | [%type: float64 ]  -> TFloat64
  | [%type: [%t? x] vector ] -> TVec (ktyp_of_typ x)
  | [%type: [%t? x] array ] -> TArr (ktyp_of_typ x, Shared)
  | [%type: bool ] -> TBool
  (*TODO ? *)
  (*  | [%type: ([%t? _] : [%t? t]) ] ktyp_of_typ t *)
  | [%type: [%t? t]] ->
    (try
       (match t.ptyp_desc  with
        | Ptyp_constr ({txt = Lident txt; _}, []) ->
          Custom ((Hashtbl.find custom_types txt),txt);
        | _ -> failwith (Format.sprintf "%s : Type not supported " (string_of_loc (t.ptyp_loc.loc_start))))
     with | _ ->
       failwith (Format.sprintf "%s : Unknown type " (string_of_loc (t.ptyp_loc.loc_start))))
...
let new_arg_of_patt p =
  ...
  | [%pat? ([%p? x] : [%t? t])] ->
    ...
    (match x.ppat_desc with
     | Ppat_var {txt; _} ->
       Hashtbl.add !current_args txt {n=i;
  ...
  | [%pat? [%p? x]] ->
    ...
    (match x.ppat_desc with
     | Ppat_var {txt; _} ->
       Hashtbl.add !current_args txt {n=i; var_type=TUnknown;
                                      is_mutable = false;
                                      read_only = false;
                                      write_only = false;
                                      is_global = false;};
...
let open_module  m_ident  loc =
  ...
  let m =
    ...
    with
    | _ -> assert (not debug); raise (Unbound_module {loc; txt = m_ident})
  in
  List.iter (fun (typ,s,_nb_args,cuda_s, opencl_s) ->
...
and close_module m_ident =
  ...
  try
    ...
    in
    List.iter (fun (_typ,s, _nb_args, _cuda_s, _opencl_s) ->
        (Hashtbl.remove !intrinsics_fun (s))) m.mod_functions;
    List.iter (fun (_typ,s,_cuda_s, _opencl_s) ->
        (Hashtbl.remove !intrinsics_const (s))) m.mod_constants;
    Hashtbl.iter (fun name _intern_m-> Hashtbl.remove modules name) m.mod_modules
  with
  | _ -> ()
...
let rec string_of_ctyp = function
  | [%type: [%t? t1] -> [%t? t2]] ->
    (string_of_ctyp t1)^" -> "^(string_of_ctyp t2)
  (* TODO ? *)
  (*  | [%type: ([%t? _] : [%t? t]) ]string_of_ctyp t *)
  (* a b *)
  | {ptyp_desc =
       Ptyp_constr (({ txt = Lident type_ident; _} ), [t2]); _} ->
    type_ident^
    " "^(string_of_ctyp t2)
  | {ptyp_desc =
       Ptyp_constr (({txt = Lident type_ident; _} ), []); _} ->
    type_ident
