...
let new_fresh_var loc =
  ...
  in let a = Ast_builder.Default.estring ~loc v in
  incr id;
  a,v
...
let rec concrete_type_to_type t =
  ...
  | [%type: Spoc.Vector.vfloat64] -> [%type: (float, Bigarray.float64_elt)  Vector.vector]
  | [%type: Spoc.Vector.vfloat32] -> [%type: (float, Bigarray.float32_elt)  Vector.vector]
  | [%type: Spoc.Vector.localvfloat32] -> [%type: (float, Bigarray.float32_elt) Vector.vector]
  | [%type: Spoc.Vector.localvfloat64] -> [%type: (float, Bigarray.float64_elt) Vector.vector]

  | [%type: Spoc.Vector.vchar] -> [%type: (char, Bigarray.int8_unsigned_elt) Vector.vector]
  | [%type: Spoc.Vector.vint32] -> [%type: (int32, Bigarray.int32_elt) Vector.vector]
  | [%type: Spoc.Vector.vint64] -> [%type: (float, Bigarray.int64_elt) Vector.vector]
  | [%type: Spoc.Vector.vbool] -> [%type: (bool,bool) Vector.vector]
  | [%type: Spoc.Vector.vcustom] -> [%type: Vector.vector]
  | [%type: Spoc.Vector.vcomplex32] -> [%type: (Complex.t, Bigarray.complex32_elt) Vector.vector]
  | [%type: Spoc.Vector.vcomplex64] -> [%type: (Complex.t, Bigarray.complex64_elt) Vector.vector]
  |(* t1 txt  *)
    {ptyp_desc =
       Ptyp_constr ({loc; txt}, [t1]);
     _} as typ  ->
    let t1 = concrete_type_to_type t1 in
    let t0 =
      concrete_type_to_type
        {typ with ptyp_desc = Ptyp_constr ({loc; txt} , [])} in
    (match t0 with
     | {ptyp_desc =
          Ptyp_constr ({loc; txt}, []); _} ->
...
let rec type_to_type t =
  ...
  | [%type: Spoc.Vector.localvfloat32] -> [%type: ('spoc_e, 'spoc_f) Vector.vector]
  | [%type: Spoc.Vector.localvfloat64] -> [%type: ('spoc_g, 'spoc_h) Vector.vector]
...
let rec parseTyp t var =
  ...
  (match t with
   ...
   | [%type: int]
   | [%type: int32] ->[%expr Spoc.Kernel.Int32 [%e i]]
   | [%type: int64] ->[%expr Spoc.Kernel.Int64 [%e i]]
   | [%type: float]
   | [%type: float32] ->[%expr Spoc.Kernel.Float32 [%e i]]

   | [%type: float64] ->[%expr Spoc.Kernel.float64 [%e i]]
   | [%type: Spoc.Vector.vint32] ->[%expr Spoc.Kernel.VInt32 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.vint64] ->[%expr Spoc.Kernel.VInt64 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.vfloat32] ->[%expr Spoc.Kernel.VFloat32 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.vfloat64] ->[%expr Spoc.Kernel.VFloat64 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.vchar] ->[%expr Spoc.Kernel.VChar (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.localvfloat32] ->[%expr Spoc.Kernel.LocalFloat32 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.localvfloat64] ->[%expr Spoc.Kernel.LocalFloat64 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.vcomplex32] ->[%expr Spoc.Kernel.VComplex32 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.vcomplex64] ->[%expr Spoc.Kernel.VComplex64 (Spoc.Kernel.relax_vector [%e i])]
   | [%type: Spoc.Vector.vbool]
   | [%type: [%t? _] Spoc.Vector.vcustom] ->[%expr Spoc.Kernel.VCustom (Spoc.Kernel.relax_vector [%e i])]
...
let gen_args loc type_list =
  ...
  let tuple =
    ...
    | a::_tl ->
      ...
      Ast_builder.Default.ppat_tuple ~loc:a.ptyp_loc l in
  (* List.fold_left
   *   (fun a b ->
  ...
  let array =
    let f t =
      ...
      parseTyp t (snd (List.nth !var_list (!idx))) (* ;
                                                    * Ast_builder.Default.evar ~loc !arg_string  *)in
    (* (
...
let relaxed loc nv =
  if not !first_vector then
    ...
    [%expr Spoc.Kernel.relax_vector [%e  Ast_builder.Default.evar ~loc nv] ]
...
let rec gen_inv_id t loc nv =
  ...
  | [%type: Spoc.Vector.vcomplex64] -> relaxed loc nv
  |(* t1 txt  *)
    {ptyp_desc =
       Ptyp_constr (_, [t1]);
     _}   ->
    gen_inv_id t1 loc nv
  | _ ->
    [%expr [%e  Ast_builder.Default.evar ~loc nv] ]
...
let rec parseInvTyp t i =
  ...
  (match t with
   ...
   | [%type: int]
   | [%type: int32] ->Longident.Lident "Int32"
   | [%type: int64] ->Longident.Lident "Int64"
   | [%type: float]
   | [%type: float32] ->Longident.Lident "Float32"
   ...
   | [%type: Spoc.Vector.vbool]
   | [%type: Spoc.Vector.vcustom] -> Longident.Lident "VCustom"
...
let gen_inv_args loc type_list =
  ...
  let tuple =
    ...
    | a::_tl ->
      let l = (List.map translate l) in
      Ast_builder.Default.pexp_tuple ~loc:a.ptyp_loc l in
  (* List.fold_left
   *   (fun a b ->
  ...
  let array =
    let f t =
      ...
      (* let arg =
       *   Ast_builder.Default.ppat_var ~loc {txt = arg; loc} in *)
      let arg2  = Ast_builder.Default.ppat_var ~loc {txt =((snd (List.nth !var_list (!idx-1)))); loc}  in
...
let gen_ktyp id_list =
  ...
  | [x] -> x
  | a::_tl ->
    Ast_builder.Default.ptyp_tuple ~loc:a.ptyp_loc ktyp
(* List.fold_left
 *   (fun (a:core_type) b ->
  ...
  object (_self)
    ...
    method! structure tp =
      ...
      List.fold_left (fun a str ->
          ...
          | Pstr_extension
              (({txt = "kernel"; _  },
                (PStr ({pstr_desc= Pstr_primitive
                            {pval_name;
                             pval_type ;
                             pval_prim = [filename; funcname]; pval_loc; _};
                        _} as _external_v ::_))), _) ->
            ...
            let k_fun = Ast_builder.Default.ppat_var  ~loc:pval_loc {txt=id; loc} in
            let exec_fun =
              let expr = Ast_builder.Default.pexp_fun ~loc Nolabel None (fst args) [%expr Spoc.Kernel.exec [%e (snd args)]] in
              Ast_builder.Default.pstr_value ~loc Nonrecursive [
                Ast_builder.Default.value_binding ~loc ~pat:k_fun ~expr:expr]
            in
            let class_name = "class_kernel_"^pval_name.txt
            and filename = Ast_builder.Default.estring ~loc filename in
            let _exec = id
            in
            let inherit_kernel = Ast_builder.Default.pcf_inherit ~loc Fresh
                (Ast_builder.Default.pcl_apply
                   ~loc
                   (* class_epxr *)
                   (Ast_builder.Default.pcl_constr ~loc {txt= Lident "spoc_kernel"; loc}
                      [k_typ; [%type:  ('a,'b) Kernel.kernelArgs array]])
                   [Nolabel, filename; Nolabel, Ast_builder.Default.estring ~loc funcname])
                None in
            let method_exec = Ast_builder.Default.pcf_method ~loc ({txt= "exec"; loc} , Public, Cfk_concrete (Fresh, Ast_builder.Default.evar ~loc id)) in
            let method_args_to_list =
              Ast_builder.Default.pcf_method
                ~loc ({txt= "args_to_list"; loc} ,
                      Public,
                      Cfk_concrete (Fresh,
                                    Ast_builder.Default.pexp_fun ~loc Nolabel None (fst args) [%expr let open Spoc.Kernel in [%e (snd args)]])) in
            ...
            let class_structure =
              Ast_builder.Default.class_structure
                ~self:[%pat? (self)]
                ~fields:[inherit_kernel; method_exec;
                         method_args_to_list; method_list_to_args] in
            let class_expr = Ast_builder.Default.pcl_structure ~loc class_structure in
            let generated_class = Ast_builder.Default.class_infos ~loc ~virt:Concrete ~params:[[%type: 'a], (NoVariance, NoInjectivity);
                                                                                               [%type: 'b], (NoVariance, NoInjectivity)] ~name:{txt=class_name; loc}  ~expr:class_expr in

            let instantiate =
              Ast_builder.Default.pstr_value ~loc Nonrecursive [Ast_builder.Default.value_binding ~loc
                                                                  ~pat:(Ast_builder.Default.ppat_var  ~loc:pval_loc pval_name)
                                                                  ~expr:(Ast_builder.Default.pexp_new ~loc {txt= Lident class_name; loc})] in
            (a@[[%stri open Spoc.Kernel];
                exec_fun;
