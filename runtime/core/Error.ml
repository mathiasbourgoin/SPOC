...
exception Transfer_error of {direction : [`To_device | `To_host]; message : string}
...
let pp_exn fmt = function
  | No_device_available ->
      Format.fprintf fmt "No compute device available"
  | Device_not_found name ->
      Format.fprintf fmt "Device not found: %s" name
  ...
  | Out_of_memory {requested; available} ->
      Format.fprintf fmt "Out of memory: requested %Ld bytes, available %Ld"
        requested available
  ...
  | Transfer_error {direction; message} ->
      Format.fprintf fmt "Transfer error (%s): %s"
        (string_of_direction direction) message
...
let try_with f =
  try ok (f ()) with exn -> error_result exn
...
let debug_vector (vec : ('a, 'b) Vector.t) =
  if should_log Trace then
    trace "Vector#%d: len=%d kind=%s loc=%s"
      (Vector.id vec)
      (Vector.length vec)
...
let debug_device (dev : Device.t) =
  if should_log Trace then
    trace "Device#%d: %s (%s) mem=%Ld"
      dev.id dev.name dev.framework
