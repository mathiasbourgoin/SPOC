...
let test_register_plugin () =
  ...
  Alcotest.(check bool) "Plugin found after registration"
    true
...
let test_list_plugins () =
  ...
  Alcotest.(check bool) "Contains MockPlugin1"
    true
    (List.mem "MockPlugin1" names)

let test_available_plugins () =
  Framework_registry.register (module Mock_plugin1);
  Framework_registry.register (module Mock_plugin2);  (* unavailable *)
  let available = Framework_registry.available () in
  let count = List.length available in
  Alcotest.(check bool) "Only available plugins listed"
    true
    (count >= 1)  (* At least MockPlugin1 *)
...
let test_priority_ordering () =
  ...
  Alcotest.(check bool) "High priority is higher than low"
    true
    (p_high > p_low)

let test_find_nonexistent () =
  let result = Framework_registry.find "NonExistentPlugin" in
  Alcotest.(check bool) "Nonexistent plugin returns None"
    true
...
let test_backend_not_found () =
  ...
  Alcotest.(check bool) "Nonexistent backend returns None"
    true
    (match result with None -> true | Some _ -> false)
