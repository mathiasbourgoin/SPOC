...
let test_register_and_find () =
  let module R = Intrinsic_registry.Make () in
  let impl = Intrinsic_registry.make_simple_intrinsic 
    ~name:"test_intr" ~codegen:"test_code" in
  R.register "test_intr" impl;
  
  let found = R.find "test_intr" in
  Alcotest.(check bool) "Intrinsic found after registration"
    true
...
let test_find_nonexistent () =
  ...
  Alcotest.(check bool) "Nonexistent intrinsic returns None"
    true
    (match found with None -> true | Some _ -> false)

let test_list_all () =
  let module R = Intrinsic_registry.Make () in
  let impl1 = Intrinsic_registry.make_simple_intrinsic 
    ~name:"intr1" ~codegen:"code1" in
  let impl2 = Intrinsic_registry.make_simple_intrinsic 
    ~name:"intr2" ~codegen:"code2" in
  
  R.register "intr1" impl1;
  R.register "intr2" impl2;
  
  let all = R.list_all () in
  Alcotest.(check bool) "list_all returns both intrinsics"
    true
    (List.length all = 2 && List.mem "intr1" all && List.mem "intr2" all)

let test_global_register () =
  let impl = Intrinsic_registry.make_simple_intrinsic
    ~name:"global_test" ~codegen:"test" in
  Intrinsic_registry.Global.register ~backend:"TestBackend" "global_test" impl;
  
  let found = Intrinsic_registry.Global.find ~backend:"TestBackend" "global_test" in
  Alcotest.(check bool) "Global intrinsic found"
    true
    (match found with Some _ -> true | None -> false)

let test_global_find_all () =
  let impl1 = Intrinsic_registry.make_simple_intrinsic
    ~name:"multi_intr" ~codegen:"code1" in
  let impl2 = Intrinsic_registry.make_simple_intrinsic
    ~name:"multi_intr" ~codegen:"code2" in
  ...
  Alcotest.(check bool) "find_all returns multiple backends"
    true
    (List.length all >= 2)

let test_backends_for () =
  let impl = Intrinsic_registry.make_simple_intrinsic
    ~name:"test" ~codegen:"code" in
  ...
  Alcotest.(check bool) "backends_for returns correct backends"
    true
    (List.mem "CUDA" backends && List.mem "OpenCL" backends)

let test_make_simple_intrinsic () =
  let impl = Intrinsic_registry.make_simple_intrinsic
    ~name:"simple" ~codegen:"simple_code" in
  Alcotest.(check string) "Name stored correctly" "simple" impl.intr_name;
  Alcotest.(check string) "Codegen stored correctly" "simple_code" impl.intr_codegen;
  Alcotest.(check bool) "Convergence is Uniform"
    true
    (impl.intr_convergence = Uniform)

let test_make_sync_intrinsic () =
  let impl = Intrinsic_registry.make_sync_intrinsic
    ~name:"barrier" ~codegen:"__syncthreads()" in
  Alcotest.(check bool) "Convergence is Sync"
    true
    (impl.intr_convergence = Sync)

let test_make_divergent_intrinsic () =
  let impl = Intrinsic_registry.make_divergent_intrinsic
    ~name:"thread_id" ~codegen:"threadIdx.x" in
  Alcotest.(check bool) "Convergence is Divergent"
    true
    (impl.intr_convergence = Divergent)

let test_is_safe_in_divergent_flow () =
  let uniform_impl = Intrinsic_registry.make_simple_intrinsic
    ~name:"uniform" ~codegen:"code" in
  let sync_impl = Intrinsic_registry.make_sync_intrinsic
    ~name:"sync" ~codegen:"barrier" in
  let divergent_impl = Intrinsic_registry.make_divergent_intrinsic
    ~name:"divergent" ~codegen:"tid" in
  
  Alcotest.(check bool) "Uniform is safe in divergent flow"
    true
    (Intrinsic_registry.is_safe_in_divergent_flow uniform_impl);
  Alcotest.(check bool) "Sync is NOT safe in divergent flow"
    false
    (Intrinsic_registry.is_safe_in_divergent_flow sync_impl);
  Alcotest.(check bool) "Divergent is safe in divergent flow"
    true
...
let () =
  ...
  run "Intrinsic_registry" [
    ...
    "convergence_safety", [
      test_case "Safe in divergent flow" `Quick test_is_safe_in_divergent_flow;
      test_case "Requires uniform execution" `Quick test_requires_uniform_execution;
    ];
    "standard_intrinsics", [
