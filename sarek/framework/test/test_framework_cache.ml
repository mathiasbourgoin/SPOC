...
let test_get_cache_dir () =
  ...
  Alcotest.(check bool) "Cache directory is not empty"
    true
    (String.length dir > 0);
  Alcotest.(check bool) "Cache directory exists"
    true
    (Sys.file_exists dir && Sys.is_directory dir)

let test_compute_key () =
  let key1 = Framework_cache.compute_key
    ~dev_name:"Device1"
    ~driver_version:"1.0"
    ~source:"kernel code" in
  
  let key2 = Framework_cache.compute_key
    ~dev_name:"Device1"
    ~driver_version:"1.0"
    ~source:"kernel code" in
  
  Alcotest.(check string) "Same inputs produce same key"
    key1 key2;
  
  Alcotest.(check bool) "Key is hex string"
    true
    (String.length key1 = 32)  (* MD5 hex is 32 chars *)
...
let test_compute_key_different () =
  ...
  Alcotest.(check bool) "Different sources produce different keys"
    true
    (key1 <> key2)
...
let test_put_and_get () =
  ...
  Alcotest.(check bool) "Retrieved data matches"
    true
...
let test_get_nonexistent () =
  ...
  Alcotest.(check bool) "Nonexistent key returns None"
    true
...
let test_stats_hit () =
  ...
  let stats = Framework_cache.get_stats () in
  Alcotest.(check bool) "Hit count incremented"
    true
    (stats.hits > 0)
...
let test_stats_miss () =
  ...
  let stats = Framework_cache.get_stats () in
  Alcotest.(check bool) "Miss count incremented"
    true
    (stats.misses > 0)
...
let test_stats_put () =
  ...
  let stats = Framework_cache.get_stats () in
  Alcotest.(check bool) "Put count incremented"
    true
    (stats.puts > 0)
...
let test_hit_rate () =
  ...
  let _ = Framework_cache.get ~key:"rate_test" in  (* Another hit *)
  let _ = Framework_cache.get ~key:"nonexistent_rate" in  (* Miss *)
  
  let rate = Framework_cache.hit_rate () in
  Alcotest.(check bool) "Hit rate is between 0 and 1"
    true
    (rate >= 0.0 && rate <= 1.0);
  
  (* Should be ~0.66 (2 hits, 1 miss) *)
  Alcotest.(check bool) "Hit rate is reasonable"
    true
    (rate > 0.5)
...
let test_cache_persistence () =
  ...
  let retrieved = Framework_cache.get ~key in
  Alcotest.(check bool) "Data persists in cache"
    true
    (match retrieved with
     | Some d -> d = data
     | None -> false)
...
let () =
  ...
  run "Framework_cache" [
    "directory", [
      test_case "Get cache directory" `Quick test_get_cache_dir;
    ];
    "key_computation", [
      test_case "Compute key" `Quick test_compute_key;
      test_case "Different inputs different keys" `Quick test_compute_key_different;
    ];
    "cache_operations", [
