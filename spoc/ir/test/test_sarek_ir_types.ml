...
let test_const_float32 () =
  ...
  (match c with CFloat32 f -> assert (abs_float (f -. 3.14) < 0.001) | _ -> assert false);
  print_endline "  const float32: OK"

let test_const_float64 () =
  let c = CFloat64 2.718281828459045 in
  (match c with CFloat64 f -> assert (abs_float (f -. 2.718281828459045) < 1e-15) | _ -> assert false);
...
let test_binop () =
  let ops = [Add; Sub; Mul; Div; Mod; Eq; Ne; Lt; Le; Gt; Ge; And; Or; Shl; Shr; BitAnd; BitOr; BitXor] in
...
let test_stmt_for () =
  let v : var = { var_name = "i"; var_id = 0; var_type = TInt32; var_mutable = true } in
...
let test_stmt_let () =
  let v : var = { var_name = "tmp"; var_id = 0; var_type = TFloat32; var_mutable = false } in
...
let test_decl_param () =
  let v : var = { var_name = "input"; var_id = 0; var_type = TVec TFloat32; var_mutable = false } in
...
let test_kernel () =
  ...
  let output_var : var = { var_name = "output"; var_id = 1; var_type = TVec TFloat32; var_mutable = false } in
  let k : kernel = {
    kern_name = "vector_add";
    kern_params = [
      DParam (input_var, Some { arr_elttype = TFloat32; arr_memspace = Global });
      DParam (output_var, Some { arr_elttype = TFloat32; arr_memspace = Global });
    ];
    kern_locals = [];
    kern_body = SEmpty;
    kern_types = [];
...
let test_helper_func () =
  let param : var = { var_name = "x"; var_id = 0; var_type = TFloat32; var_mutable = false } in
  let hf : helper_func = {
    hf_name = "square";
    hf_params = [param];
