...
external%kernel gpuPi :
  point Spoc.Vector.vcustom -> Spoc.Vector.vint32 -> int -> float -> unit
  = "kernels/CustomPi" "pi"

external%kernel gpuPi_complex :
  Spoc.Vector.vcomplex32 -> Spoc.Vector.vint32 -> int -> float -> unit
  = "kernels/CustomPi" "pi"

external%kernel gpuPi_double :
  point Spoc.Vector.vcustom -> Spoc.Vector.vint32 -> int -> float -> unit
  = "kernels/CustomPi" "pi_double"

let cpuPI field size =
  let resVec =
    Array.map (fun pt -> (pt.x *. pt.x) +. (pt.y *. pt.y) <= ray *. ray) field
  in
  let res = Array.fold_left (fun i res -> if res then i + 1 else i) 0 resVec in
  res
...
let gpuPi_complex vbool dev size =
  ...
  let threadsPerBlock =
    ...
    | _ -> 256
  in
  let blocksPerGrid = ((size / 32) + threadsPerBlock - 1) / threadsPerBlock in
  let block =
    {
  ...
  Spoc.Kernel.run
    ...
    (gpuField, vbool, nbPoint, ray *. ray) ;
  let pio4 =
    Int32.to_int (Tools.fold_left (fun a b -> Int32.add a b) Int32.zero vbool)
  in
  Printf.printf
    "GPU Complex Computation : PI = %d/%d = %.10g\n%!"
    pio4
    size
...
let multiGpuPI gpuField1 gpuField2 vbool1 vbool2 dev1 dev2 size =
  ...
  let blocksPerGrid =
    ((size / 32 / 2) + threadsPerBlock - 1) / threadsPerBlock
  in
  let block =
    {
      Spoc.Kernel.blockX = threadsPerBlock;
  ...
  let pio4_1 =
    Int32.to_int (Tools.fold_left (fun a b -> Int32.add a b) Int32.zero vbool1)
  and pio4_2 =
    Int32.to_int (Tools.fold_left (fun a b -> Int32.add a b) 0l vbool2)
  in
  let pio4 = pio4_1 + pio4_2 in
  pio4
...
let _ =
  ...
  and arg2 =
    ( "-size",
      Arg.Int (fun i -> size := i),
      "width of the image to compute [2_000_000]" )
  in
  Arg.parse [arg1; arg2] (fun s -> ()) "" ;
  if Devices.allowDouble !dev then Printf.printf "Will use double precision\n"
  else
    Printf.printf "Will use simple precision, results may be innacurate ;-)\n" ;
  let pio4 = ref 0 in
  (*CPU COMPUTATION*)
  let field =
    Array.init !size (fun _ -> {x = Random.float ray; y = Random.float ray})
  in
  pio4 := cpuPI field !size ;
  Printf.printf
    "CPU Computation : PI = %d/%d = %.10G\n"
    !pio4
    !size
    (4. *. float !pio4 /. float !size) ;
