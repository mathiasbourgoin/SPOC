...
[%%kernel
external vec_add :
  Spoc.Vector.vfloat32 ->
  Spoc.Vector.vfloat32 ->
  Spoc.Vector.vfloat32 ->
  int ->
  unit = "kernels/VecAdd" "vec_add"]

[%%kernel
external vec_add_double :
  Spoc.Vector.vfloat64 ->
  Spoc.Vector.vfloat64 ->
  Spoc.Vector.vfloat64 ->
  int ->
  unit = "kernels/VecAdd" "vec_add_double"]
...
let _ =
  ...
  and arg2 =
    ( "-size",
      Arg.Int (fun i -> vec_size := i),
      "size of the vectors to multiply [1024]" )
  and arg3 =
    ( "-auto",
      Arg.Bool (fun b -> auto_transfers := b),
      "let Spoc handles transfers automatically [false]" )
  and arg4 =
    ("-check", Arg.Bool (fun b -> verify := b), "check computation [true]")
  in
  ...
  if allow_double then begin
    ...
    if not !auto_transfers then begin
      ...
      Spoc.Mem.to_device res !dev (* Kernel launch : computation *)
    end ;
    begin
    ...
    if !verify then (
      ...
      begin
        for i = 0 to Spoc.Vector.length res - 1 do
          ...
          if tmp -. Spoc.Mem.get res i > 1.e-8 then begin
            Printf.printf
              "ERROR Index: %d - %g <> %g ----> %g\n"
              i
              tmp
              (Spoc.Mem.get res i)
              (tmp -. Spoc.Mem.get res i) ;
            Printf.printf
              "a[%d] = %g, b[%d] = %g\n%!"
              i
              (Spoc.Mem.get a i)
        ...
        if !correct then Printf.printf "Verif OK\n"
        else Printf.printf "Verif KO\n"
      end)
  end
  else begin
    ...
    if !verify then (
      ...
      begin
        for i = 0 to Spoc.Vector.length res - 1 do
          let tmp = Spoc.Mem.get a i +. Spoc.Mem.get b i in
          if tmp -. Spoc.Mem.get res i > 1.e-5 then begin
            Printf.printf
              "ERROR Index: %d - %g <> %g ----> %g\n"
              i
              tmp
