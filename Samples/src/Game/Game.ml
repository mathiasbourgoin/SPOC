...
external%kernel game : Spoc.Vector.vint32 -> int -> int -> int -> unit
  = "kernels/Game" "game"
...
let couleur n = if n = 1 then Graphics.red else Graphics.blue

let main () =
  Random.self_init () ;
  let arg0 =
    ( "-device",
      Arg.Int (fun i -> dev1 := devices.(i)),
      "number of the device [0]" )
  and arg1 =
    ( "-width",
      Arg.Int (fun i -> largeur := i),
      "width of the image to compute [1024]" )
  and arg2 =
    ( "-height",
      Arg.Int (fun i -> hauteur := i),
      "height of the image to compute [1024]" )
  and arg3 =
    ( "-gens",
      Arg.Int (fun b -> max_gen := b),
      "max number of generations [1024]" )
  in
  Arg.parse [arg0; arg1; arg2; arg3] (fun s -> ()) "" ;
  let b_iter1 =
    Spoc.Vector.create
      Spoc.Vector.int32
      (*`Int32 Bigarray.int32*) (!largeur * !hauteur)
  and img1 = Array.make_matrix !largeur !hauteur Graphics.black in
  Printf.printf
    "Will use device : %s\n%!"
  ...
  in
  let blocksPerGrid =
    ((!largeur * !hauteur) + threadsPerBlock - 1) / threadsPerBlock
  in
  let block =
    {
  ...
  for i = 0 to Spoc.Vector.length b_iter1 - 1 do
    if Random.int 2 = 1 then Mem.set b_iter1 i (Random.int32 (Int32.of_int 2))
    else Mem.set b_iter1 i (Int32.of_int 0)
  done ;
  ...
  let draw tab img dev queue =
    Spoc.Tools.iteri
      (fun elt i ->
        ...
        img.(b).(a) <- couleur (Int32.to_int elt))
      tab ;
  ...
  Spoc.Kernel.run
    ...
    (block, grid)
    game
    (b_iter1, !max_gen, !largeur, !hauteur) ;
