...
external%kernel game : Spoc.Vector.vint32 -> int -> int -> int -> unit = "kernels/Game" "game"
...
let main () =
  ...
  for i = 0 to Spoc.Vector.length b_iter1 - 1 do
    (if (Random.int 2) = 1 then
       Mem.set b_iter1 i (Random.int32 (Int32.of_int 2))
     else
       Mem.set b_iter1 i (Int32.of_int 0));
  done;
