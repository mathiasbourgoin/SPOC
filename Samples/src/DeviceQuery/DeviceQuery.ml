...
Array.iteri
  (fun i dev ->
    ...
    | Spoc.Devices.CudaInfo ci ->
        ...
        Printf.printf
          "    Cuda %d.%d compatible\n"
          ci.Spoc.Devices.major
          ci.Spoc.Devices.minor ;
        ...
        let thread = ci.Spoc.Devices.maxThreadsDim in
        Printf.printf
          "    Max Threads Dim : %dx%dx%d\n"
          thread.Spoc.Devices.x
          thread.Spoc.Devices.y
          thread.Spoc.Devices.z ;
        let grid = ci.Spoc.Devices.maxGridSize in
        Printf.printf
          "    Max Grid Size : %dx%dx%d\n"
          grid.Spoc.Devices.x
          grid.Spoc.Devices.y
          grid.Spoc.Devices.z ;
        Printf.printf
    ...
    | Spoc.Devices.OpenCLInfo cli ->
        let platform = cli.Spoc.Devices.platform_info in
        Printf.printf
          "    OpenCL compatible (via Platform : %s)\n"
        ...
        | Spoc.Devices.CL_DEVICE_TYPE_DEFAULT -> Printf.printf "DEFAULT\n") ;
        ...
        Printf.printf
          "    Memory Base Addr Align : %d\n"
          cli.Spoc.Devices.mem_base_addr_align ;
        Printf.printf
          "    Min Data Type Align Size : %d\n"
        ...
        | Spoc.Devices.CL_FP_ROUND_TO_ZERO -> Printf.printf "FP ROUND TO ZERO\n"
        | Spoc.Devices.CL_FP_ROUND_TO_INF -> Printf.printf "FP ROUND TO INF\n"
        ...
        | Spoc.Devices.CL_READ_WRITE_CACHE -> Printf.printf "READ WRITE CACHE\n"
        | Spoc.Devices.CL_READ_ONLY_CACHE -> Printf.printf "READ ONLY CACHE\n"
        ...
        for j = 0 to Spoc.Devices.cuda_devices () - 1 do
          if
            String.compare
              dev.Spoc.Devices.general_info.Spoc.Devices.name
              devices.(j).Spoc.Devices.general_info.Spoc.Devices.name
            = 0
          then
            Printf.printf
