...
(** Vector Copy Benchmark

    Pure memory bandwidth test - the simplest possible kernel. Measures raw
    memory transfer performance with minimal compute overhead. Baseline for
    comparing bandwidth-limited vs compute-limited kernels. *)
...
let benchmark_device dev size config =
  ...
  (* Get kernel IR *)
  let _, kirc = vector_copy_kernel in
  let ir =
  ...
  (* Copy result back *)
  let result = Vector.to_array vb in
...
let run_benchmark config =
  Printf.printf "Vector Copy Benchmark\n" ;
  Printf.printf "=====================\n\n" ;
